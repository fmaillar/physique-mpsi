# -*- coding: utf-8 -*-
"""
Created on Sat Aug 20 14:37:34 2022
...
axs[0].annotate("Cutoff, $G =$" + str(np.around(mag[IC], decimals=0)),
                xy=(w[IC], mag[IC]), xytext=(w[IC]+3, mag[IC]),
...
axs[1].annotate("Cutoff, $\Phi =$" + str(np.around(phase[IC], decimals=0)),
                xy=(w[IC], phase[IC]), xytext=(w[IC]+3, phase[IC]),
                arrowprops=dict(facecolor='black', shrink=0.05, width=0.2, frac=0.2))
