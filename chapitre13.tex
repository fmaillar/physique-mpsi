...
\section{Lois de Kirchhoff, associations de dipôles, équivalences}
    ...
    En parallèle : Les dipôles sont sous la même tension et les intensités s'ajoutent. Pour les grandeurs complexes associées, on a donc~: $\ic = \sum_k {\ic}_k = \sum{k} {\yc}_k \uc $. \emph{L'admittance complexe du dipôle équivalent est la somme des admittances complexe des dipôles en parallèle}~: $\Yc = \sum_{k=1}^n \Yc_k$.

    En particulier pour deux dipôles en parallèle : $\Yc = \Yc_1 + \Yc_2$, d'où $\Zc = \frac{\Zc_1 \Zc_2}{\Zc_1+\Zc_2}$. Par exemple, l'admittance complexe d'un condensateur de capacité $C$ avec une conductance de fuite non nulle $G$ est~: $\Yc = G + \ju\omega C$, donc $Y = \sqrt{G^2 + C^2 \omega^2}$,
